...
The remaining question is how do we rotate the edge directions to obtain the normals? Recall that the 2D clockwise rotation matrix is:
\begin{equation}
\begin{bmatrix}
\cos \theta & \sin \theta \\
-\sin \theta & \cos \theta
\end{bmatrix},
\end{equation}
where $\theta$ is the rotation angle -- plugging in $\theta = 90^{\circ}$ we obtain:
\begin{equation}
\begin{bmatrix}
0 & 1 \\
-1 & 0
