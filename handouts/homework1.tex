...
\section{Rendering multiple circles (20 pts)}
...
\section{Rendering more shapes (20 pts)}
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{\label{fig:transformation} The five affine transformations we will support in this homework. Notice that only the translation cannot be expressed using a $2 \times 2$ matrix.}
\end{figure}
...
Therefore, we can represent all affine transformation using the $3 \times 3$ matrix above. \cref{fig:transformation_affine} shows the corresponding version of the matrices for each transformation.
