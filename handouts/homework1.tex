...
For this part, you can choose to implement either style above that suits you better. The scene is given in the array \lstinline{hw1_2_scenes} in \lstinline{hw1_scenes.h}. We will select the scenes using command line arguments. Implement your rendering code in the function \lstinline{hw_1_2} and see the result using the following commands:
\begin{lstlisting}[language=bash]
...
% In this part, we will extend our previous code to handle multiple circles. We will mainly address two challenges: 1) Specifying multiple circles through the command line is cumbersome. How can we make this easier? 2) What do we do when a pixel center hits more than one circle? Which color should we take?

% For challenge 1), we will introduce a \emph{scene file} that represents the objects we want to render. There is no a single standard way to represent a scene in computer graphics.\footnote{Though in 2D, \href{https://en.wikipedia.org/wiki/SVG}{Scalable Vector Graphics (SVG)} is the most popular, and in 3D, studios are converging towards the \href{https://en.wikipedia.org/wiki/Universal_Scene_Description}{Universal Scene Description}.} Thus, we will use an easily readable and parsible \href{https://en.wikipedia.org/wiki/JSON}{JSON} format to describe our scene. It's easiest to explain by showing an example of the scene:
